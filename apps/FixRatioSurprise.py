...
def fix_ratio_surprise(box_name, initial, progressive, time_s, log_dir, mouse_name, session=1, light_on_lever=False,
               startup_sequence=True, active_lever='LL'):
    if initial < 1:
        ...
        def on_event(event, pi_timestamp):
            ...
            if startup_sequence:
                ...
                if ll_startup and rl_startup:
                    ...
                    bonsai_sender.send_message('/flexratio',
                                               f'{mouse_name},"video_start",-1,{time.time()},{str(datetime.now(pytz.utc))}')
            ...
            if event == active_lever:
                ...
                if state['active_lever_presses'] == state['next_reward']:
                    dispense_pellet(box)
                    food_light_blink(box)
                    log_writer.writerow([mouse_name, state['next_event'], pi_timestamp, timestamp, datetimestamp])
                    bonsai_sender.send_message('/flexratio',
                                               f'{mouse_name},"{state["next_event"]}",{pi_timestamp},{timestamp},{datetimestamp}')
                    state['active_lever_presses'] = 0
                    if state['n_pellet'] % 7 == 2:
                        state['next_reward'] = 6
                        state['next_event'] = 'FD-short'
                    elif state['n_pellet'] % 7 == 5:
                        state['next_reward'] = 40
                        state['next_event'] = 'FD-long'
                    else:
                        state['next_reward'] = initial
                        state['next_event'] = 'FD'
                    state['n_pellet'] += 1
            if light_on_lever:
                if event == 'LL':
...
if __name__ == '__main__':
    fix_ratio_surprise('box_4', 2, 0, 60, '', 'B1234', 1, True)
