...
def flex_ratio_reversed(box_name, initial, progressive, time_s, log_dir, mouse_name, session=1, light_on_lever=False, startup_sequence=True, active_lever='LL'):
    if initial < 1:
        ...
        def on_event(event, pi_timestamp):
            ...
            if light_on_lever:
                ...
                if event == 'RL':
                    ...
                    time.sleep(0.01)
                    right_light_off(box)
            if state['next_reward'] == 25: # change to other lever and PR2 after the 4th pellet
                print('levers reversed')
                active_lever = 'RL'
                progressive = 2
                state['next_reward'] = 1
...
if __name__ == '__main__':
    flex_ratio_reversed('box_4', 2, 2, 30, '', 'B1234', 1, True)
