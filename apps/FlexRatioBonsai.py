...
def flex_ratio(box_name, initial, progressive, time_s, log_dir, mouse_name, session=1, startup_sequence=True, active_lever='LL'):
    if initial < 1:
    ...
    with open(Path(log_dir) / '{m}_{p}_{s}.log'.format(m=mouse_name, p=progressive, s=session), 'w', newline='') as f:
        ...
        def on_event(event, pi_timestamp):
            ...
            timestamp = time.time()
            datetimestamp = str(datetime.now(pytz.utc))
